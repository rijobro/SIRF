...
Options:
  -s <file>, --sino=<file>      template sinogam (default: mMR/mMR_template_span11_small.hs)
  -i <path>, --img=<path>       template image (default: auto-generated by sinogram)
    ...
    for z in range(centre[0] - radius, centre[0] + radius + 1):
        for y in range(centre[1] - radius, centre[1] + radius + 1):
            for x in range(centre[2] - radius, centre[2] + radius + 1):
                dist = pow((centre[0]-z)**2 + (centre[1]-y)**2 + (centre[2]-x)**2, 0.5)
                if dist <= radius:
...
def main():
    ...
    if any(dims < min_dims):
        raise pet.error(f'Image too small. Minimum size: {min_dims}, actual size: {dims}')
    # Get centre of image
    centre = dims//2
    ...
    for i in count_fractions:
        ...
        noisy_sino.write(f_out_sino + "_" + str(i).replace('.','_') + "-percent")
