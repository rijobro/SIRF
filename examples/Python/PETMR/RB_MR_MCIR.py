...
for ind in range(num_ms):
    acq_ms_sim[ind] = pMR.AcquisitionData(data_path + '/raw_T1_ms' + str(ind) + '.h5')
...
for ind in range(num_ms):
    ...
    AcqModMs[ind] = pMR.AcquisitionModel(acq_ms_sim[ind], template_mr_im)
...
for ind in range(num_ms):
    ...
    temp_fwd = AcqModMs[ind].direct(motion_T1s[ind])
    print("testing back projection...")
    temp_bck = AcqModMs[ind].adjoint(temp_fwd)

print("tesing norm")
for ind in range(num_ms):
    print("ind = " + str(ind))
    temp_norm = acq_ms_sim[ind].norm()
...
for ind in range(num_ms):
    ...
    if hasattr(x0, 'squared_norm'):
        print("im here5")
        x1.dot(x0) / x0.squared_norm()
    else:
        print("im here6")
        x0norm = x0.norm()
        print("im here7")
        x1.dot(x0) / (x0norm * x0norm) 
    print("im here8")
    x1.multiply((1.0/x1norm), out=x0)
    print("im here9")

    
raise AssertionError("all good1")
